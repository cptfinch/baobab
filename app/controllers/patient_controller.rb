...
#      raise session[:encounter_datetime].to_date.class.to_s
      ...
      @show_lab_trail = true if (@user.activities.include?("HIV Staging") ||  @user.activities.include?("ART Visit")) and GlobalProperty.find_by_property("show_lab_trail").property_value rescue "false"
    end
        ...
        case concept_name
          ...
          when "Is able to walk unaided"
           ...
           visits.keys.each{|date|
               ...
               pills_given=patient_obj.drug_orders_for_date(date)
               total_pills_given=0
               number_of_pills_given=pills_given.collect{|dor|total_pills_given+=dor.quantity if dor.drug.name.to_s !="Insecticide Treated Net" and dor.drug.name.to_s !="Cotrimoxazole 480"}.compact if pills_given
               unless  number_of_pills_given.nil? || number_of_pills_given.empty?
                  drug_names = pills_given.collect{|dor|dor.drug.name.to_s + "</br>" if dor.drug.name.to_s !="Insecticide Treated Net" and dor.drug.name.to_s !="Cotrimoxazole 480"}.compact.uniq
                  visits[date].reg = drug_names.to_s
                  ...
                  visits[date].gave = "G" + "</br>" + number_of_pills_given.last.to_s if drugs_given_to_guardian
                  visits[date].gave = "P" + "</br>" + number_of_pills_given.last.to_s if drugs_given_to_patient
                  visits[date].gave = "PG" + "</br>" + number_of_pills_given.last.to_s if drugs_given_to_both_patient_and_guardian
                  if !drugs_given_to_guardian and !drugs_given_to_patient and !drugs_given_to_both_patient_and_guardian
                    visits[date].gave =  number_of_pills_given.last
                  end 
               end 
            }
...
# Only 4000 arv numbers allowed at MPC
 ...
 unless @previous_height.blank? or @previous_weight.blank?
  ...
  if needs_cd4_count_reminder == "true" and (User.current_user.has_role("Nurse") ||  User.current_user.has_role("Clinician") || User.current_user.has_role("superuser"))
   ...
   @show_lab_trail =  GlobalProperty.find_by_property("show_lab_trail").property_value rescue "false"
  else
  ...
  def create_paper_mastercard_patients
    Patient.create_patients 
    flash[:notice] = "New patients created!!"
    redirect_to :action =>"paper_mastercard"
  end
