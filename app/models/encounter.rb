...
#      raise prescription.to_yaml if prescription.quantity.nil?
  ...
  def self.patients_encountered_for_date(date)
    Encounter.find(:all, :include => "patient", :conditions => ["DATE(encounter_datetime) = ?",date]).collect{|e|e.patient}.uniq
  end
...
# If they are a transfer in with a letter we want the receptionist to copy the staging info using the retrospective staging form
  ...
  def self.art_patients_encountered_for_date(date)
    Encounter.find(:all, :include => "patient", :conditions => ["DATE(encounter_datetime) = ?",date]).collect{|e|e.patient if e.patient.art_patient?}.compact.uniq
  end
  ...
  def self.number_patients(date) 
    enc_type=EncounterType.find_by_name("HIV Reception").id
    return Encounter.count(:all,:conditions => ["DATE(encounter_datetime) = ? and encounter_type=?",date,enc_type])
  end
  ...
  def self.count_total_number(date) 
   ...
   def self.count_total_number_vitals_taken(date)
   enc_type=EncounterType.find_by_name("Height/Weight").id
   return Encounter.find(:all,:include => "patient",:conditions => ["DATE(encounter_datetime) = ? and encounter_type=?",Date.today,enc_type]).collect{|pat|
   unless pat.patient_id.nil?
     pat.patient_id
   end
   }.uniq.compact.length
  end
  ...
  def parse_observations(params)
    ...
    params["observation"].each{|type_and_concept_id,answer|
      ...
      if concept_id.to_i == Concept.find_by_name('Provider').id
        self.provider_id = User.find_by_username(answer).id
        next
      end
      observation = self.add_observation(concept_id)
      need_save = true
