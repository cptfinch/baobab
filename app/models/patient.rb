  ...
  class Patient < OpenMRS
	...
	#       find(:first,:conditions => ["encounter_type = ?", EncounterType.find_by_name(type_name).id], :order => "encounter_datetime DESC, date_created DESC")
	#    end
    ...
    def last_encounter_by_flow(date = Date.today)
	  ...
	  def self.create_patients
	    PaperMastercard.create_patients
	    #return Time.now().strftime("%Y-%m-%d")
	  end
	  
	  def self.create_pih_patients
	    PihDatas.create_openmrs_patients
	    #return Time.now().strftime("%Y-%m-%d")
	  end

	  def next_forms(date = Date.today)

	    return if self.outcome != Concept.find_by_name("On ART")
...
## DRUGS
	  def cohort_last_art_regimen(start_date=nil, end_date=nil)
	    ...
	    dispensation_encounters = self.encounters.find(
	      :all, 
	      :conditions => ["encounter_type = ? AND DATE(encounter_datetime) >= ? AND DATE(encounter_datetime) <= ?", dispensation_type_id, start_date, end_date],
	      :order => "encounter_datetime DESC"
	    )
	    return nil if dispensation_encounters.empty?
	    dispensation_encounters.each {|encounter| 
...
## DRUGS
    ...
    def adult?
	  ...
	  def date_started_art(regimen_type = "ARV First line regimen")
	    ...
	    medication_dispensations = self.encounters.find_by_type_name("Give drugs")
	    medication_dispensations.each{|medication_dispensation|
	      unless DrugOrder.drug_orders_to_regimen(medication_dispensation.drug_orders).blank?
		  arv_dispensing_dates << medication_dispensation.encounter_datetime if DrugOrder.drug_orders_to_regimen(medication_dispensation.drug_orders).name == regimen_type
	      else
#		puts "#{self.id}" + "#{regimen_type}"	      
	      end
	    }     
	    return arv_dispensing_dates.sort.first unless arv_dispensing_dates.nil?
	  end
	  ...
	  def ARV_national_id
	    begin
	      self.patient_identifiers.find_by_identifier_type(PatientIdentifierType.find_by_name("Arv national id").id).identifier 
	    rescue
	     return nil
	   end
	  end
	  ...
	  def arv_number=(value)
	    ...
	    arv_number.patient = self
	    arv_number.type = PatientIdentifierType.find_by_name("Arv national id")
	    prif=value.match(/(.*)[A-Z]/i)[0]
	    number=value.match(/[0-9](.*)/i)[0].to_i
	    arv_number.identifier= prif + " " + number.to_s
	    arv_number.save
	  end
...
## DRUGS
	  ...
	  # Check for defaulters every day to make sure the defaulter observations occur as soon as someone becomes a defaulter
	  def self.update_defaulters_range(start_date, end_date)
	    (start_date...end_date).to_a.collect{|date|
	      print "."
	      Patient.update_defaulters(date)
	    }
	  end
...
## DRUGS
	  def art_amount_remaining_if_adherent(from_date)
	    drug_orders = self.previous_art_drug_orders(from_date)
...
## DRUGS
	...
	# return just the values that were stored in the hash (since we used the hash to remove duplicates)
	  ...
	  def Patient.find_by_birth_traditional_authority(ta)
	    ta_type_id = PatientIdentifierType.find_by_name("Birth traditional authority").patient_identifier_type_id
	    return PatientIdentifier.find(:all, :conditions => ["identifier_type = ? AND identifier LIKE ?",ta_type_id,"%#{ta}%"]).collect{|patient_identifier| patient_identifier.patient} 
	  end
	  ...
	  def Patient.find_by_filing_number(number)
	    filing_id_type = PatientIdentifierType.find_by_name("Filing number").patient_identifier_type_id
	    return PatientIdentifier.find(:all,:conditions => ["identifier_type =?  and identifier LIKE ?",filing_id_type, "%#{number}%"]).collect{|patient_identifier| patient_identifier.patient}
	  end
	...
	# find all patient name objects that have a given_name or family_name like the value passed in
	  ...
	  def Patient.find_patients_no_staging
	    return Patient.find_all.collect{|pat|
	      if(pat.encounters.find_by_type_name("HIV Staging").empty? and ! pat.filing_number.nil?)
		pat.filing_number
	      end
	    }.compact
	  end
	...
	#................................................
	  ...
	  def  Patient.find_patients_with_followups_child
	    enc_type=EncounterType.find_by_name("Height/Weight").id
	    return Patient.find_all.collect{|pat|
	    if( ! pat.encounters.find_by_type_name("Height/Weight").empty? )
	     count= Encounter.count_by_sql "SELECT count(*) FROM openmrs.encounter where patient_id=#{pat.patient_id} and encounter_type=#{enc_type}  and encounter_datetime >= '2007-02-01' and encounter_datetime <= '2007-02-28'"
	      if count > 1 and pat.child?  then
		 pat.patient_id
	      end
	     end
	   }.compact
	 ...
	 def  Patient.find_patients_with_followups_adults(sex,start_date,end_date)
	   enc_type=EncounterType.find_by_name("Height/Weight").id
	   return Patient.find_all.collect{|pat|
	    if( ! pat.encounters.find_by_type_name("Height/Weight").empty? )
	     count= Encounter.count_by_sql "SELECT count(*) FROM openmrs.encounter where patient_id=#{pat.patient_id} and encounter_type=#{enc_type} and encounter_datetime >= '#{start_date}' and encounter_datetime <= '#{end_date}'"  
	     if count > 1   and ! pat.child? and pat.gender==sex then
	       pat
	     end
	    end
	   }.compact   
	   ...
	   def Patient.find_patients_with_followups
	   return Patient.find_all.collect{|pat|
	     if( ! pat.encounters.find_by_type_name("ART Visit").empty? and ! pat.filing_number.nil?)
	       pat.filing_number
	     end
	     }.compact
	   end
	   ...
	   def Patient.find_patients_child(month,year)
	     return Patient.find_all.collect{|pat|
	     if ! pat.filing_number.nil? and pat.child? and pat.date_created.year ==year and pat.date_created.month==month
		 pat.patient_id
	      end
	     }.compact
	   end
	  ...
	  def Patient.find_patients_no_staging2
	       enc=Encounter.find_all_by_encounter_type(EncounterType.find_by_name("HIV Staging").id)
	       ids=enc.collect{|id|id.patient_id}
	       sqls="SELECT identifier FROM patient_identifier where identifier_type=10 "
	       ids.each{|id|
		 sqls=sqls + " and patient_id <> " + id.to_s
	       }
	       number=PatientIdentifier.find_by_sql sqls
	       return number.collect{|n|n.identifier} 
	  end
	  
	  
	  def Patient.find_patients_with_no_date_of_positive_hiv_test
	  return Patient.find_all.collect{|pat|
	    if(pat.observations.find_by_concept_name("Date of first positive HIV test").empty? and ! pat.filing_number.nil?)
	      pat.filing_number
	    end
	   }.compact
	  end 

	   def Patient.find_patients_with_no_dates_of_art_initiation
	   return Patient.find_all.collect{|pat|
	     if(pat.observations.find_by_concept_name("Date of ART initiation").empty? and ! pat.filing_number.nil?)
		pat.filing_number
	     end
	     }.compact
	   end
	   ...
	   def  Patient.count_total_number_of_vitals_taken 
	      return Encounter.count_total_number_vitals_taken(Date.today)  
	   end
	  ...
	  def Patient.virtual_register
		...
		Observation.find(:all,:group=>"obs.patient_id",:order => "obs_datetime desc").collect{|obs|
		  ...
		  patient_observations = Observation.find(:all,:conditions => ["concept_id=? and patient_id=?",(Concept.find_by_name(concept_name).id),pat.patient_id],:order=>"obs.obs_datetime desc")
		    case concept_name
	...
	#added objects are: weight and side effects
		    when "Weight"
			  ...
			  patient_register[hash_key].last_weight = patient_observations.last.value_numeric.nil? ? "Unavailable" :  patient_observations.last.value_numeric
			  patient_register[hash_key].first_weight = patient_observations.first.value_numeric.nil? ? "Unavailable" :  patient_observations.first.value_numeric
		      end 
	  ...
	  def Patient.art_clinic_name(location_id)
	   ...
	   def Patient.find_patients_no_f
	     return Patient.find_all.collect{|pat|
	      if pat.filing_number.nil? or pat.filing_number==""
	       pat.filing_number
	      end
	    }.compact
	  end
...
## DRUGS
	  ...
	  def place_of_first_hiv_test 
	    ...
	    name = Location.find(location_id).name unless location_id.nil?
	    return name  
	  end
			...
			while i < total_encounters
				...
				if this_encounter.name == "ART Visit" and not followup_done
					this_encounter.observations.each { |o|
						...
						#next if this_concept == "PTB within the past 2 years"
						# default is value_coded observation
						cohort_visit_data[this_concept] = o.value_coded == 3 #unless this_concept == "PTB within the past 2 years"  # 3 is the concept_id for 'Yes'
					}
					followup_done = true
    ...
    def self.cabinets_filing_number_labels(filing_number_type = "active")
	    label = ZebraPrinter::StandardLabel.new
      number = 5000
      number2 = 5099
      cabinet_number = ""
      version_number = 1
      filing_area = Location.current_arv_code + " " + filing_number_type
      while number2 <= 9999
       cabinet_number = (number.to_s + " - " + number2.to_s)
	     label.draw_text("#{cabinet_number}",75, 30, 0, 4, 4, 4, false)            
	     label.draw_text("Filing area: #{filing_area}",75, 150, 0, 2, 2, 2, false)            
	     label.draw_text("Version number: #{version_number.to_s}",75, 220, 0, 2, 2, 2, false)            
	     label.print(1)
	     label.header if number2 < 9999
       number+= 100 
       number2+= 100 
      end
	    return label.print(1)
...
## DRUGS
    def drug_dispensed_label(date=Date.today)
	    ...
	    next_appointment_date="Next appointment date: #{next_appointment.strftime("%d-%b-%Y")}" unless next_appointment.nil?
	    symptoms = Array.new
	    weight=nil
      ...
      concept_names.each{|concept_name|
	   ...
	   visit_by="Visit by both patient and guardian" if self.patient_and_guardian_present?(date)
     visit_by="Patient visit" if self.patient_present?(date) and !self.guardian_present?(date)
	   visit_by="Guardian visit" if self.guardian_present?(date) and !self.patient_present?(date)
     ...
     label.draw_multi_text("Current Data: #{weight}; #{amb}; #{work_sch}; #{symptom_text} #{adherence}",{:font_reverse =>false})
     label.draw_multi_text("Drugs given: #{drugs_given}",{:font_reverse =>false})
     label.draw_multi_text("Outcome: #{current_outcome}, #{next_appointment_date}",{:font_reverse => false})
	   return label.print(2)
	  end
...
## DRUGS
	  def self.addup_prescride_drugs(priscriptions)
	    prescride_drugs=Hash.new()

	    priscriptions.each{|p|
	      priscription =  p.split(",")
	      drug_name = priscription[0]
	      frequency = priscription[1]
	      dose_amount = priscription[2]
	      prescride_drugs[drug_name]=Hash.new() if prescride_drugs[drug_name].blank?
	      prescride_drugs[drug_name] = "#{frequency}: #{dose_amount}  "  if prescride_drugs[drug_name].blank?
	      prescride_drugs[drug_name] << "#{frequency}: #{dose_amount}"  if !prescride_drugs[drug_name].blank? and !prescride_drugs[drug_name].match(/#{frequency}/i)
	    }
	    #return prescride_drugs
      drugs_given = ""
	    prescride_drugs.each do |drug_name,prescriptions|
	      drugs_given+="#{drug_name} (1-0-1) " if  "#{prescriptions.to_s.strip}".match(/morning/i) and  "#{prescriptions.to_s.strip}".match(/evening/i) and !"#{prescriptions.to_s.strip}".match(/noon/i)
	      drugs_given+="#{drug_name} (0-0-1) " if  !"#{prescriptions.to_s.strip}".match(/morning/i) and  "#{prescriptions.to_s.strip}".match(/evening/i) and !"#{prescriptions.to_s.strip}".match(/noon/i)
	      drugs_given+="#{drug_name} (0-1-0) " if  !"#{prescriptions.to_s.strip}".match(/morning/i) and  !"#{prescriptions.to_s.strip}".match(/evening/i) and "#{prescriptions.to_s.strip}".match(/noon/i)
	      drugs_given+="#{drug_name} (0-1-1) " if  !"#{prescriptions.to_s.strip}".match(/morning/i) and  "#{prescriptions.to_s.strip}".match(/evening/i) and "#{prescriptions.to_s.strip}".match(/noon/i)
	      drugs_given+="#{drug_name} (1-1-0) " if  "#{prescriptions.to_s.strip}".match(/morning/i) and  !"#{prescriptions.to_s.strip}".match(/evening/i) and "#{prescriptions.to_s.strip}".match(/noon/i)
	      drugs_given+="#{drug_name} " if  "#{prescriptions.to_s.strip}".match(/prescription/i)
	    end
      return drugs_given
	  end

	  def self.create_from_mastercard(data)
	    patient = Patient.new
	    patient.save
...
#End of HL 7 functions
  ...
  def self.empty_cohort_data_hash
    cohort_values = Hash.new

    cohort_values["all_patients"] = 0
    cohort_values["male_patients"] = 0
    cohort_values["female_patients"] = 0
    cohort_values["adult_patients"] = 0
    cohort_values["child_patients"] = 0
    cohort_values["pmtct_pregnant_women_on_art"] = 0
    cohort_values["ambulatory_patients"] = 0
    cohort_values["working_patients"] = 0
    cohort_values["adherent_patients"] = 0

    cohort_values["peripheral_neuropathy_patients"] = 0
    cohort_values["hepatitis_patients"] = 0
    cohort_values["skin_rash_patients"] = 0
    cohort_values["lactic_acidosis_patients"] = 0
    cohort_values["lipodystropy_patients"] = 0
    cohort_values["anaemia_patients"] = 0
    cohort_values["other_side_effect_patients"] = 0
   
    cohort_values["on_1st_line_with_pill_count_adults"] = 0
    cohort_values["defaulters"] = 0

    cohort_values["start_cause_PTB"] = 0
    cohort_values["start_cause_APTB"] = 0
    cohort_values["start_cause_EPTB"] = 0
    cohort_values["start_cause_KS"] = 0
   
    cohort_values["regimen_types"] = Hash.new
    cohort_values["dead_patients"] = 0
    cohort_values["alive_on_ART_patients"] = 0
    cohort_values["transferred_out_patients"] = 0
    cohort_values["art_stopped_patients"] = 0
   
    cohort_values["died_1st_month"] = 0
    cohort_values["died_2nd_month"] = 0
    cohort_values["died_3rd_month"] = 0
    cohort_values["died_after_3rd_month"] = 0
  ...
  def cohort_data(start_date, end_date, cohort_values=nil)
    ...
    cohort_values["all_patients"] += 1 
      ...
      cohort_values["start_cause_KS"] += 1 if cohort_visit_data["Kaposi's sarcoma"] == true
      pmtct_obs = self.observations.find_by_concept_name("Referred by PMTCT").last
      cohort_values["pmtct_pregnant_women_on_art"] +=1 if pmtct_obs.value_coded == 3 unless pmtct_obs.nil?
      ...
      if outcome_status == "Died" 
        ...
        unless self.death_date.blank?
          ...
          if months_of_treatment <= 1 then cohort_values["died_1st_month"] += 1 
          elsif months_of_treatment == 2 then cohort_values["died_2nd_month"] += 1
          elsif months_of_treatment == 3 then cohort_values["died_3rd_month"] += 1
          elsif months_of_treatment > 3 then cohort_values["died_after_3rd_month"] += 1
          end
        else
        ...
        if (regimen_type)
          cohort_values["regimen_types"][regimen_type.name] ||= 0
          cohort_values["regimen_types"][regimen_type.name] += 1
          cohort_values["ambulatory_patients"] += 1 if cohort_visit_data["Is able to walk unaided"] == true
          cohort_values["working_patients"] += 1 if cohort_visit_data["Is at work/school"] == true

          if patient_started_as_adult and regimen_type.name == "ARV First line regimen" and not cohort_visit_data["Pill count"].nil?
            cohort_values["on_1st_line_with_pill_count_adults"] += 1 
            cohort_values["adherent_patients"] += 1 if cohort_visit_data["Pill count"] <= 8
          end
        else
        ...
        # Side effects
        cohort_values["peripheral_neuropathy_patients"] += 1 if cohort_visit_data["Peripheral neuropathy"]
        cohort_values["hepatitis_patients"] += 1 if cohort_visit_data["Hepatitis"]
        cohort_values["skin_rash_patients"] += 1 if cohort_visit_data["Skin rash"]
        cohort_values["lactic_acidosis_patients"] += 1 if cohort_visit_data["Lactic acidosis"]
        ...
        cohort_values["other_side_effect_patients"] += 1 if cohort_visit_data["Other side effect"]
      end
      return cohort_values
