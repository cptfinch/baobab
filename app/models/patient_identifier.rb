...
class PatientIdentifier < OpenMRS
  ...
  def self.update(patient_id, identifier, identifier_type, reason)    
    current = PatientIdentifier.find_by_patient_id_and_identifier_type(patient_id, identifier_type, :conditions => "voided = 0")
    return if current and current.identifier == value
    current.void! reason if current     
    new_obj = current ? current.composite_clone : PatientIdentifier.new
