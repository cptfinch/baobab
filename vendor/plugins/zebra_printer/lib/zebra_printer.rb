...
module ZebraPrinter #:nodoc:
  ...
  class Label 
    ...
    def initialize(width = 776, height = 329, orientation = 'T')
      @width = width || 776
      @height = height || 329
    ...
    #     1= 6pts (8 x 12 dots)
    #     2= 7pts (10 x 16 dots)
    #     3= 10pts (12 x 20 dots)
    #     4= 12pts (14 x 24 dots)
    #     5= 24pts (32 x 48 dots)
      ...
      data.split("\n").each {|line|
        ...
        while word_start_index + word_count < words.size 
          ...
          if (size + next_word_size > @column_width)
            ...
            text = words[word_start_index..(word_start_index + (word_count-1))].join(" ")
            check_bounds
  ...
  private
    ...
    def get_word_size(char_width, word, need_space)
      char_width * (word.size + (need_space ? 1 : 0))
    end
  end
